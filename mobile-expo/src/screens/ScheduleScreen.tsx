...
function useScheduleData() {
  ...
  const [intakeLogsByScheduleId, setIntakeLogsByScheduleId] = useState<
    Record<number, MedicationIntakeLogResponse[]>
  >({});
  ...
  return (
    <>
      <Screen>
        ...
        <SectionCard title={selectedDate} subtitle="선택한 날짜의 회차별 복약 목록입니다.">
          {loading ? (
            <Text style={mobileStyles.mutedText}>일정을 불러오는 중입니다.</Text>
          ) : doseGroups.length ? (
            <View style={{ gap: 10 }}>
              {doseGroups.map((group) => {
                ...
                return (
                  <View
                    key={group.key}
                    style={[mobileStyles.doseCard, isComplete ? mobileStyles.doseCardComplete : null]}
                  >
                      ...
                      <View style={mobileStyles.doseActionRow}>
                        <Pressable
                          ...
                          style={[
                            ...
                            actionLoading || isComplete ? mobileStyles.completeButtonDisabled : null,
                          ]}
                        >
