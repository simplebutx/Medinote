...
1. `.env.docker`가 없을 때만 예시 파일을 복사해서 로컬 환경변수 파일을 만듭니다.

   ```powershell
   if (!(Test-Path .env.docker)) { Copy-Item .env.docker.example .env.docker } else { Write-Host ".env.docker already exists; skip copy" }
   ```

   이미 `.env.docker`가 있으면 이 파일에는 개인 API 키가 들어 있을 수 있으니 덮어쓰지 않습니다.
...
## `qdrant-snapshot-import` 서비스를 찾을 수 없을 때

아래 명령으로 현재 받은 compose 파일에 snapshot 서비스가 있는지 확인합니다.

```powershell
docker compose --env-file .env.docker --profile snapshot config --services
```

목록에 `qdrant-snapshot-import`가 없으면 최신 코드를 못 받은 상태이거나, 프로젝트 루트가 아닌 다른 폴더에서 명령을 실행한 것입니다. `docker-compose.yml`이 있는 프로젝트 루트에서 최신 브랜치/커밋을 받은 뒤 다시 실행합니다.
