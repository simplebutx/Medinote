...
const PharmacistProfile = () => {
    ...
    const [activeTab, setActiveTab] = useState('basic') // basic, verification, pharmacy
    ...
    const loadProfile = async () => {
        try {
            ...
            try {
                ...
                if (pharmRes.data) {
                    setPharmacyForm({
                        pharmacyName: data.docNumber || pharmRes.data.name || '',
                ...
                setPharmacyForm(prev => ({ ...prev, pharmacyName: data.docNumber || '' }))
    ...
    const handleSaveBasic = async () => {
        try {
            ...
            loadProfile()
        } catch (error) { alert('기본 정보 수정에 실패했습니다.') }
    }
    ...
    const handleSaveLicense = async () => {
        try {
            ...
            alert('면허 정보가 수정되었으며, 재승인이 필요합니다.')
            setIsEditingLicense(false)
            loadProfile()
        } catch (error) { alert('면허 정보 수정에 실패했습니다.') }
    }
    ...
    const handleSavePharmacy = async () => {
        try {
            const payload = {
                ...pharmacyForm,
                pharmacyName: profile.docNumber,
            ...
            alert(error.response?.data || '약국 정보 저장 실패')
        }
    }
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 20px' }}>
                ...
                <div style={sectionCardStyle}>
                    ...
                    {isEditingBasic ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div style={inputGroupStyle}><label style={labelStyle}>이름</label><input style={inputStyle} value={basicForm.username} onChange={e => setBasicForm({...basicForm, username: e.target.value})} /></div>
                            <div style={inputGroupStyle}><label style={labelStyle}>생년월일</label><input type="date" style={inputStyle} value={basicForm.birthDate} onChange={e => setBasicForm({...basicForm, birthDate: e.target.value})} /></div>
                            <div style={inputGroupStyle}><label style={labelStyle}>성별</label>
            ...
            {activeTab === 'pharmacy' && (
                <div style={sectionCardStyle}>
                    ...
                    {isEditingPharmacy ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div style={inputGroupStyle}><label style={labelStyle}>상세 주소</label><input style={inputStyle} value={pharmacyForm.address} onChange={e => setPharmacyForm({...pharmacyForm, address: e.target.value})} /></div>
                            <div style={inputGroupStyle}><label style={labelStyle}>전화번호</label><input style={inputStyle} value={pharmacyForm.phone} onChange={e => setPharmacyForm({...pharmacyForm, phone: e.target.value})} /></div>
                            <div style={gridInfoStyle}>
                                <div style={inputGroupStyle}><label style={labelStyle}>위도</label><input type="number" step="any" style={inputStyle} value={pharmacyForm.latitude} onChange={e => setPharmacyForm({...pharmacyForm, latitude: e.target.value})} /></div>
                                <div style={inputGroupStyle}><label style={labelStyle}>경도</label><input type="number" step="any" style={inputStyle} value={pharmacyForm.longitude} onChange={e => setPharmacyForm({...pharmacyForm, longitude: e.target.value})} /></div>
                            </div>
                                    ...
                                    {DAYS.map(day => (
                                        <div key={day.key} style={{ padding: '12px', border: '1px solid #e2e8f0', borderRadius: '10px' }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                                <input type="time" style={timeInputStyle} value={formatTimeForInput(pharmacyForm[`${day.key}Open`])} onChange={e => setPharmacyForm({...pharmacyForm, [`${day.key}Open`]: formatTimeForApi(e.target.value)})} />
                                                <span>~</span>
                                                <input type="time" style={timeInputStyle} value={formatTimeForInput(pharmacyForm[`${day.key}Close`])} onChange={e => setPharmacyForm({...pharmacyForm, [`${day.key}Close`]: formatTimeForApi(e.target.value)})} />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div style={infoItemVerticalStyle}><span>약국명</span><strong>{profile.docNumber}</strong></div>
