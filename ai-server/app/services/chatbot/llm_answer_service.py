...
# llm한테 보낼 컨텍스트 빌드
def build_llm_context(question: str,results: list[dict],drug_name: str | None = None,schedule_context: str = "", ) -> tuple[str, bool]:
    context_blocks: list[str] = []
    total_chars = 0
    has_grounded_match = False
    ...
    return "\n".join(prompt_lines), has_grounded_match

# llm에게 답변 요청
def generate_answer_from_context(question: str,results: list[dict],drug_name: str | None = None, schedule_context: str = "",) -> str:
    prompt_input, has_grounded_match = build_llm_context(
        question=question,
        results=results,
        drug_name=drug_name,
    ...
    # 응답
    answer = normalize_answer_text(response.choices[0].message.content or "")
    if answer:
        if schedule_context.strip() and answer.startswith("답변드리기 어렵습니다."):
            answer = answer.removeprefix("답변드리기 어렵습니다.").strip()
        if not has_grounded_match and not schedule_context.strip() and not answer.startswith("답변드리기 어렵습니다."):
            answer = f"답변드리기 어렵습니다. {answer}"
        return answer

    if schedule_context.strip():
        return "현재 등록된 복약 일정 정보를 바탕으로 답변을 정리하지 못했어요. 다시 질문해 주세요."

    if not has_grounded_match:
        subject = f"{drug_name} 문서" if drug_name else "문서"
        return f"답변드리기 어렵습니다. {subject}에서 질문과 직접 관련된 내용을 찾기 어려웠습니다."

    subject = f"{drug_name} 문서" if drug_name else "근거 문서"
    return f"{subject}를 바탕으로 답변을 만들지 못했어요. 잠시 후 다시 시도해 주세요."
